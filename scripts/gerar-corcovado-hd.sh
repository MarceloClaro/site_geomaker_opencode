#!/usr/bin/env bash
# ============================================================================
# gerar-corcovado-hd.sh
# Gera o modelo 3D de relevo em papel do Corcovado, Rio de Janeiro
# em ALTA RESOLUÇÃO MILIMETRADA (240×192).
#
# Características:
#   - Grade 240×192 (vs 160×128 original) — 90% mais pontos
#   - Exagero vertical z_cms=10 (vs 8) — mais relevo
#   - Comprimento length_cm=14 (vs 12) — fatias mais largas
#   - Gap 0.01cm (0.1mm) — precisão sub-milimétrica
#   - Suavização 3 passadas
#   - Detecção de pico
#   - Mapa OSM + PDF instruções
#
# Uso: ./gerar-corcovado-hd.sh [--force]
...
SAIDAS_DIR="/home/marceloclaro/Geomaker_site/saidas/corcovado-240x192"
...
# Remove diretório anterior se --force
if [ -d "$SAIDAS_DIR" ] && [ "${1:-}" == "--force" ]; then
...
log "=============================================="
log "GERAÇÃO DO MODELO — CORCOVADO HD 240×192"
log "=============================================="
log "Local: $PLACE"
log "Área: ${SIZE_KM}×${SIZE_KM} km"
log "Resolução: ${LAT_STEPS}×${LON_STEPS}"
log "Altura (z): ${Z_CMS}cm | Comprimento: ${LENGTH_CM}cm"
log "Gap: ${GAP_CM}cm | Suavização: ${SMOOTH_PASSES} passadas"
log "Saída: $SAIDAS_DIR"
log ""

# Verificar se o cache interpolado existe
CACHE_FILE=$(find "$MODERNIZED_DIR/.elevation_cache" -name "*.json" -newer "$SCRIPTS_DIR/interpolar-elevacoes.rb" 2>/dev/null | head -1)
if [ -z "$CACHE_FILE" ]; then
    warn "Cache HD pode não existir. Executando interpolação..."
    ruby "$SCRIPTS_DIR/interpolar-elevacoes.rb" "$LAT_STEPS" "$LON_STEPS"
fi

# 1. Gerar o modelo principal (SVG visão geral + páginas A4)
log "Gerando modelo de relevo HD..."
cd "$MODERNIZED_DIR"
...
log "Modelo HD gerado com sucesso!"

# 2. Copiar metadados e ajustar
log "Ajustando metadados..."
cp "$SAIDAS_DIR"/metadados.json "$SAIDAS_DIR"/metadados.json.bak

# 3. Gerar mapa interativo OSM
...
log ""
log "=============================================="
log "✅ MODELO DO CORCOVADO HD COMPLETO!"
log "=============================================="
log "Acesse: http://localhost:8080/saidas/corcovado-240x192/"
log ""
...
checks['6. Páginas A4'] = len(pages) >= 20
...
echo ""
echo "Pronto! O modelo Corcovado HD está disponível em:"
echo "  http://localhost:8080/saidas/corcovado-240x192/"
echo ""
